...
OXAPAY_MERCHANT_KEY = "WA8VLX-MBKRAG-Z8XWAN-69ZG2N"
...
OXAPAY_WEBHOOK_URL = "https://worker-production-5f8a.up.railway.app/oxapay-webhook"
            ...
            try:
                await app.bot.send_message(
                    ...
                    text=(
                        ...
                        "Deposit funds — the rule stays active for your next post."
                    ),
                    parse_mode=ParseMode.MARKDOWN,
                )
            except Exception as e:
...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    keyboard = [
        ...
        [InlineKeyboardButton(get_text("btn_autopost"), callback_data="autopost")],
        [
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "services":
        ...
        # If user has a pending deposit, show a quick Verify button
        pending = get_pending_deposit(user_id)
        if pending:
            wallet_kb.insert(0, [InlineKeyboardButton("🔄 Verify Pending Payment", callback_data="verify_payment")])
        ...
        await query.edit_message_text("🔍 Checking OxaPay for your deposit, please wait...")
        pending = get_pending_deposit(user_id)
        ...
        track_id = pending.get("code")
        usd_amount = pending.get("usd", pending.get("token", 0))
        print(f"🔍 Verifying: user={user_id}, track_id={track_id}, usd={usd_amount}")
        ...
        try:
            info = await check_oxapay_invoice(track_id)
            oxapay_status = info.get("status", "?")
            oxapay_result_code = info.get("result", "?")
            print(f"📦 OxaPay response: result={oxapay_result_code}, status={oxapay_status}")

            if oxapay_status.lower() in ("paid", "confirming"):
                # Use actual received USD amount if OxaPay reports it (overpayment case)
                actual_usd = float(info.get("payAmount") or info.get("receivedAmount") or info.get("paidAmount") or usd_amount)
                # payAmount is in crypto — use 'amount' field which is USD if available
                actual_usd_final = float(info.get("amount") or usd_amount)
                # Take whichever is higher — invoice amount or what OxaPay actually received
                credit_amount = round(max(actual_usd_final, usd_amount), 4)
                print(f"💰 Crediting: invoice=${usd_amount}, oxapay_amount=${actual_usd_final}, final=${credit_amount}")
                if _claim_tx_hash(track_id):
                    _finalize_deposit(user_id, credit_amount, track_id, "OxaPay Autopay", credit_amount)
                    result = (credit_amount, credit_amount, track_id)
                else:
                    result = (credit_amount, credit_amount, track_id)
        except Exception as e:
            error_msg = str(e)
            print(f"⚠️ OxaPay check error: {e}")

        # OxaPay invoice stays valid for 30 minutes — no attempt limit needed.
        # User can tap Verify/Try Again as many times as they want until it's paid or expired.
        ...
        if result:
            ...
            text = (
                ...
                f"🔎 *OxaPay Status:* `{oxapay_status}` (result: `{oxapay_result_code}`)\n"
                f"🆔 *Track ID:* `{track_id}`\n\n"
                "Tap *Try Again* after paying."
            )
            if error_msg:
                text += f"\n\n⚠️ *Error:* `{error_msg}`"
        await query.edit_message_text(text=text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
        ...
        services = get_custom_services("telegram")
        if not services:
            text = "📂 *Telegram Services*\n\nNo Telegram services added yet. Ask the admin to add one first."
            kb = InlineKeyboardMarkup([
        ...
        text = (
            ...
            "Step 1/3 — Pick the Telegram service to auto-order on every new post:"
        )
        await query.edit_message_text(text=text, reply_markup=InlineKeyboardMarkup(rows), parse_mode=ParseMode.MARKDOWN)
        return
